...
int main(int argc, char** argv) {
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    */
    ...
    Body jbody_sub[N/size];
    MPI_Win win;
    MPI_Win_create(jbody_sub, N/size*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    MPI_Win_fence(0, win);
    MPI_Put(jbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    std::memcpy(jbody, jbody_sub, N/size*sizeof(Body));
